...
import { ParsedInvoiceItem } from '../models';

@Component({
  selector: 'app-invoice-recognition',
  standalone: true,
    ...
    <div class="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
      <div class="bg-white dark:bg-primary p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-xl font-bold">Adicionar via Nota Fiscal (IA)</h3>
        ...
        <div class="flex-grow overflow-y-auto pr-2 text-center">
          @if (isLoading()) {
            <div class="flex flex-col items-center justify-center h-full">
              ...
              <p class="text-lg font-semibold">Analisando nota fiscal...</p>
              <p class="text-slate-500 dark:text-slate-400 mt-2">A IA está lendo os itens do documento.</p>
            </div>
          } @else if (capturedImage()) {
            <div class="flex flex-col items-center">
              <p class="mb-4">Foto capturada. Deseja analisar esta imagem?</p>
              <img [src]="capturedImage()" alt="Captured invoice" class="rounded-lg max-h-80 mb-4 border-2 border-slate-300 dark:border-secondary" />
              <div class="flex gap-4">
                <button (click)="retakePhoto()" class="px-4 py-2 bg-slate-200 dark:bg-secondary rounded">Tirar Outra Foto</button>
                <button (click)="analyzeImage()" class="px-4 py-2 bg-accent text-white rounded">Analisar Nota Fiscal</button>
              </div>
            </div>
          } @else {
            <div class="flex flex-col items-center">
              <p class="mb-4">Posicione a nota fiscal em um local bem iluminado e tire uma foto nítida.</p>
...
export class InvoiceRecognitionComponent implements AfterViewInit, OnDestroy {
  invoiceRecognized = output<ParsedInvoiceItem[]>();
  close = output<void>();
  ...
  async analyzeImage() {
    ...
    try {
      const result = await this.geminiService.analyzeInvoiceImage(base64Data);
      if (result) {
        this.invoiceRecognized.emit(result);
      } else {
        // Error toast is already shown by the service, allow user to retry
        this.retakePhoto();
      }
    } finally {
